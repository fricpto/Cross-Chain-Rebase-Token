...
cast send ${ZKSYNC_REBASE_TOKEN_ADDRESS} --rpc-url ${ZKSYNC_SEPOLIA_RPC_URL} --account Myaccount --legacy --zksync "grantMintAndBurnRole(address)" ${ZKSYNC_POOL_ADDRESS}
echo "Pool permissions set"

# Set the CCIP roles and permissions
echo "Setting the CCIP roles and permissions on ZKsync..."
cast send ${ZKSYNC_REGISTRY_MODULE_OWNER_CUSTOM} "registerAdminViaOwner(address)" ${ZKSYNC_REBASE_TOKEN_ADDRESS} --rpc-url ${ZKSYNC_SEPOLIA_RPC_URL} --account Myaccount --legacy --zksync
cast send ${ZKSYNC_TOKEN_ADMIN_REGISTRY} "acceptAdminRole(address)" ${ZKSYNC_REBASE_TOKEN_ADDRESS} --rpc-url ${ZKSYNC_SEPOLIA_RPC_URL} --account Myaccount --legacy --zksync
cast send ${ZKSYNC_TOKEN_ADMIN_REGISTRY} "setPool(address,address)" ${ZKSYNC_REBASE_TOKEN_ADDRESS} ${ZKSYNC_POOL_ADDRESS} --rpc-url ${ZKSYNC_SEPOLIA_RPC_URL} --account Myaccount --legacy --zksync
...
# setting roles on sepolia
echo "Setting the roles for the pool contract on Sepolia..."
forge script ./script/Deployer.s.sol:SetRole --rpc-url ${SEPOLIA_RPC_URL} --account Myaccount --broadcast --sig "run(address,address)" ${SEPOLIA_REBASE_TOKEN_ADDRESS} ${SEPOLIA_POOL_ADDRESS}
echo "Pool roles set"

# setting registry module owner custom on sepolia
echo "Setting the admin for the pool contract on Sepolia..."
forge script ./script/Deployer.s.sol:SetRegistryModuleOwnerCustom --rpc-url ${SEPOLIA_RPC_URL} --account Myaccount --broadcast --sig "run(address)" ${SEPOLIA_REBASE_TOKEN_ADDRESS} 
echo "registry module owner custom set"

# accepting the admin role on sepolia
echo "Accepting the admin role for the pool contract on Sepolia..."
forge script ./script/Deployer.s.sol:SetTokenAdminRegistryToAcceptAdminRole --rpc-url ${SEPOLIA_RPC_URL} --account Myaccount --broadcast --sig "run(address)" ${SEPOLIA_REBASE_TOKEN_ADDRESS}
echo "Admin role accepted"

# setting the pool on sepolia
echo "Setting the pool for the token admin registry on Sepolia..."
forge script ./script/Deployer.s.sol:SetTokenAdminRegistryToSetPool --rpc-url ${SEPOLIA_RPC_URL} --account Myaccount --broadcast --sig "run(address,address)" ${SEPOLIA_REBASE_TOKEN_ADDRESS} ${SEPOLIA_POOL_ADDRESS}
echo "Pool set"
...
cast send ${ZKSYNC_POOL_ADDRESS}  --rpc-url ${ZKSYNC_SEPOLIA_RPC_URL} --account Myaccount "applyChainUpdates(uint64[],(uint64,bytes[],bytes,(bool,uint128,uint128),(bool,uint128,uint128))[])" "[${REMOTE_CHAIN_SELECTORS_TO_REMOVE}]" "[(${SEPOLIA_CHAIN_SELECTOR},[$(cast abi-encode "f(address)" ${SEPOLIA_POOL_ADDRESS})],$(cast abi-encode "f(address)" ${SEPOLIA_REBASE_TOKEN_ADDRESS}),(false,0,0),(false,0,0))]"
